...
// Define interfaces for event handlers
interface DragInfo {
  offset: {
    x: number;
    y: number;
...
const RollingGallery: React.FC<RollingGalleryProps> = ({
  ...
  const handleUpdate = (latest: LatestValues) => {
  ...
  const handleDrag = (
    _: React.MouseEvent | React.TouchEvent | PointerEvent,
    info: DragInfo
  ) => {
    controls.stop();
      ...
      <div className="flex h-full items-center justify-center [perspective:1000px] [transform-style:preserve-3d]">
        ...
        >
          {displayImages.map((url, i) => (
            ...
            >
              {/* Using regular img tag for better 3D transform compatibility */}
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={url}
                alt={`Skill gallery image ${i + 1}`}
